                ...
                <ul className="flex gap-6 " >
                    {
                        headerData.map((data:headerDataType) => (
                            <Link to={`/${link}/`+data.link} key={data.id} className={`py-4 cursor-pointer hover:text-black text-grey_text ${(`/${link}/`+data.link) === location.pathname && 'border-b text-black border-primary'}`}  >{data.title}</Link>
                        ))
                    }
                </ul>
            </nav>
