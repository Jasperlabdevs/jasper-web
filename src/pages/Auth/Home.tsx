    ...
    return(
        <div className="md:mt-40 mt-16 mx-auto text-center w-full md:min-w-[430px] max-w-[620px] ">
            ...
            <div className="text-left md:mt-20 mt-10 " >
                {
                    roles.map(data => (
                        <Link to={`${data.link}`} className="p-4 my-8 w-full border rounded-md cursor-pointer flex justify-between items-center" key={data.id}>
                            <div>
                                ...
                                <p className="text-grey_text text-xs lg:text-sm leading-5 lg:leading-6" >Are you a {data.role}? <br /> Sign Up as {data.type} here.</p>
                            </div>
                           <div>
                               {SVGs.arrow_right}
                           </div>
